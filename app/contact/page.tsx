...
export default function ContactPage() {
	...
	const handleShipIt = async (e: React.FormEvent) => {
		...
		try {
			...
			if (response.ok) {
				alert("TRANSMISSION_SUCCESS: Message relayed to Moksh.");
				setFormData({ alias: "", email: "", message: "" }); // Clear form
			} else {
				alert("TRANSMISSION_FAILURE: Packet dropped at gateway.");
			}
		} catch (err) {
			alert("CRITICAL_ERROR: Connection timed out.");
		}
	};
	...
	return (
		<div className="min-h-screen bg-black text-green-500 font-mono flex flex-col items-center justify-center p-6">
			...
			<Card className="w-full max-w-lg bg-zinc-950 border-green-800 shadow-[0_0_20px_rgba(34,197,94,0.15)] mt-20">
